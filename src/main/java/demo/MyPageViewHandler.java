    ...
    public void whenDeliveryCanceled_then_UPDATE_2(@Payload DeliveryCanceled deliveryCanceled) {
        try {
            if (deliveryCanceled.isMe()) {
                ...
                System.out.println("### order Id :" + deliveryCanceled.getOrderId());
                System.out.println("### getStatus :" + deliveryCanceled.getStatus());

                // view 객체 조회
                List<MyPage> myPageList = myPageRepository.findByOrderId(deliveryCanceled.getOrderId());
                for(MyPage myPage : myPageList){

                    System.out.println("### myPage order Id :" + myPage.getOrderId());
                    System.out.println("### myPage getStatus :" + myPage.getStatus());
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
